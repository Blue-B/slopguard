...
(`https://slopguard.app` or a custom
...
precision 100% / recall 92%. Adding an LLM key lifts recall on the subtlest
cases. Everything is configurable via `.github/SLOP_POLICY.yml`.

It's free and MIT to self-host. There's also a hosted version (we pay the LLM
bill): Free for public repos, Pro $19/mo for private repos + dedicated quota,
Team $99/mo for org controls. Same open-core model as Sentry/PostHog.
...
- Works with zero API keys (heuristics-only: precision 100% / recall 92% on a
  labeled set). Add an LLM key for higher recall.
- Free + MIT to self-host; hosted Pro ($19/mo) if you'd rather not run it.
...
LLM 없이도 작동합니다(휴리스틱-only: 정밀도 100% / 재현율 92%, 라벨링된 25개
골든셋 기준). LLM 키를 넣으면 재현율이 올라갑니다. .github/SLOP_POLICY.yml로 임계값,
라벨, allowlist를 설정합니다.

MIT 라이선스로 직접 호스팅하면 무료입니다. 직접 운영하기 싫다면 호스팅 버전도 있습니다
(공개 레포 무료, Pro $19/월 비공개 레포, Team $99/월 조직 기능).
